...
//    app.post("hello",
//             name: "hello") { req -> ParseHookResponse<String> in
...
//    app.post("score", name: "gamescore") { req async throws -> ParseHookResponse<GameScore> in
...
//     }
    ...
    // A Parse Hook Trigger route.
//    app.post("lokality", name: "lokality") { req async throws -> ParseHookResponse<Lokality> in
//         // Note that `ParseHookResponse<GameScore>` means a "successfull"
//         // response will return a "GameScore" type.
...
//         }
//         var parseRequest = try req.content
//             .decode(ParseHookTriggerObjectRequest<User, Lokality>.self)
//
//         // If a User called the request, fetch the complete user.
//         if parseRequest.user != nil {
//             parseRequest = try await parseRequest.hydrateUser(options: [.usePrimaryKey], request: req)
...
//         // To query using the primaryKey pass the `usePrimaryKey` option
//         // to ther query.
//         let scores = try await Lokality.query.first(options: [.usePrimaryKey])
//         req.logger.info("Lokality: \(scores)")
...
//              //: Lokality.className,
//              //triggerName: .
//     ) { req async throws -> ParseHookResponse<Lokality> in
//         // Note that `ParseHookResponse<[GameScore]>` means a "successfull"
//         // response will return a "[GameScore]" type.
...
//         var parseRequest = try req.content
//             .decode(ParseHookFunctionRequest<User, Lokality>.self)
...
//        // To query using the primaryKey pass the `usePrimaryKey` option
//        // to ther query.
//        let lokality = try await Lokality.query.first(options: [.usePrimaryKey])
       
//     //    let options = try parseRequest.options(req)
//     //    let lokality = try await Lokality.query.first(options: options)
//        req.logger.info("Scores this user can access: \(lokality)")
